...
impl SystemHandles {
    pub fn system_handle_management(
        ...
        let mut split_acceptor_control_chan_receivers = acceptor_control_chan_receivers.handle_split();
        ...
        let mut hashmap_client_replica_broadcast_chan_receivers: HashMap<usize, Vec<Receiver<u8>>> = HashMap::new();
        ...
        let mut hashmap_leader_acceptor_broadcast_chan_receivers: HashMap<usize, Vec<Receiver<u8>>> = HashMap::new();
        ...
        for client_id in 0..client_count {
            ...
            // build the client
            let client_context = client::new( client_id as u8, 
            ...
            hashmap_client_replica_broadcast_chan_receivers.insert(
                client_id,
                split_client_replica_broadcast_chan_receivers
            ); 
        ...
        for leader_id in 0..leader_count {
            ...
            hashmap_leader_acceptor_broadcast_chan_receivers.insert(
                leader_id,
                split_leader_acceptor_broadcast_chan_receivers
            );
        ...
        for acceptor_id in 0..acceptor_count {
            ...
            for leader_id in 0..leader_count {
                ...
                hashmap_leader_acceptor_broadcast_chan_receivers.insert(
                    leader_id,
                    hashmap_entry_leader_id
                );
            }

            // build the acceptor
            let acceptor_context = acceptor::new(acceptor_id as u8, 
                                            leader_acceptor_broadcast_chan_receivers, 
    ...
    pub fn activate_broadcast(&self, 
        num_broadcasts: u8, 
        client_count: u8, 
        replica_count: u8,
        leader_count: u8,
        ...
        self.client_control_chan_sender.send(client::ControlSignal::Run(num_broadcasts));
        self.replica_control_chan_sender.send(replica::ControlSignal::Run(client_count*num_broadcasts));
        self.leader_control_chan_sender.send(leader::ControlSignal::Run(replica_count*client_count*num_broadcasts));
        self.acceptor_control_chan_sender.send(acceptor::ControlSignal::Run(leader_count*replica_count*client_count*num_broadcasts));
    
        thread::sleep(Duration::from_secs(5));
        println!("Exit signal sent; hopefully, clients, leaders, replicas, acceptors in paused mode");
        self.client_control_chan_sender.send(client::ControlSignal::Exit);
        ...
        self.acceptor_control_chan_sender.send(acceptor::ControlSignal::Exit);
...
mod tests {
    ...
    fn it_works() {
        ...
        let num_msgs = 2u8;
        let system_handles = SystemHandles::system_handle_management(client_count, 
                                                                    replica_count, 
                                                                    leader_count,
                                                                    acceptor_count
                                                                );
        system_handles.activate_broadcast(num_msgs, 
                                        client_count as u8,     
                                        replica_count as u8,
                                        leader_count as u8
                                    );
